...
class DB{
    ...
    public function registreer($firstname, $tsv, $lastname, $email, $uname, $psw){
        ...
        try{
            // insert new account
            $query1 = "INSERT INTO account (username, email, password, created, updated, usertype_id) VALUES (:username, :email, :wachtwoord, CURRENT_TIMESTAMP , CURRENT_TIMESTAMP, 2)";
    ...
    public function checkRole(){
        ...
        $this->stmt = $this->dbh->prepare("SELECT usertype_id FROM account WHERE email = :email");
        $result = $this->stmt->execute(
            array(
